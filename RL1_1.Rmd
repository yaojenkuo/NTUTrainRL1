...
# iris
library(magrittr)
irisColLength <- iris %>% names() %>% length()
for (i in 1:irisColLength){
  print(names(iris)[i])
}
...
status <- c("Poor", "Improved", "Excellent", "Poor") %>% factor(., ordered = TRUE, levels = c("Poor", "Improved", "Excellent"))#建立狀況
# statusFactor <- factor(status, ordered = TRUE, levels = c("Poor", "Improved", "Excellent"))
